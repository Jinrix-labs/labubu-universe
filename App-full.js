...
function CollectionScreen({ user, onBrowse, onBack }) {
  ...
  const handleAddPhoto = async (labubuId) => {
    console.log('handleAddPhoto called for:', labubuId);
  ...
  const uploadPhoto = async (labubuId, uri) => {
    ...
    try {
      console.log('Starting photo upload for:', labubuId, 'URI:', uri);
      ...
      Alert.alert('Upload Failed', `There was an error uploading your photo: ${error.message}`);
    } finally {
      setUploadingId(null);
    }
  };
          ...
          renderItem={({ item }) => (
            <View style={styles.labubuCard}>
              <Image
                ...
                onError={(error) => {
                  console.log('Image load error for', item.id, ':', error.nativeEvent.error);
                }}
                onLoad={() => {
            ...
            renderItem={({ item }) => (
              ...
              >
                <Image
                  ...
                  onError={(error) => {
                    console.log('Photo Studio image load error for', item.labubuId, ':', error.nativeEvent.error);
                  }}
                  onLoad={() => {
                    console.log('Photo Studio image loaded successfully for', item.labubuId);
                  }}
                />
